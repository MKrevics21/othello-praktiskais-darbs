    ...
    for y in range(BOARD_SIZE):
        for x in range(BOARD_SIZE):
            if is_valid_move(board, x, y, player):
                return True
    return False if player == HUMAN_PLAYER else has_valid_moves(board, HUMAN_PLAYER)
...
def minimax(board, depth, maximizing_player, player):
    ...
    if maximizing_player:
        ...
        for y in range(BOARD_SIZE):
            for x in range(BOARD_SIZE):
                if is_valid_move(board, x, y, player):
                    ...
                    evaluation = minimax(new_board, depth - 1, False, player)
        ...
        for y in range(BOARD_SIZE):
            for x in range(BOARD_SIZE):
                if is_valid_move(board, x, y, 3 - player):
                    new_board = deepcopy(board)
                    make_move(new_board, x, y, 3 - player)
                    evaluation = minimax(new_board, depth - 1, True, player)
                    min_eval = min(min_eval, evaluation)
        return min_eval

#Pārbauda kurā brīdi lietotājs vai dators ir zaudējis
def game_over(board):
    human_has_moves = has_valid_moves(board, HUMAN_PLAYER)
    computer_has_moves = has_valid_moves(board, COMPUTER_PLAYER)
    
    if not (human_has_moves or computer_has_moves):
        return True
    else:
        return False
...
#Pārbauda, vai spēle ir neizšķirta
def is_tie(board):
    for y in range(BOARD_SIZE):
        for x in range(BOARD_SIZE):
            if board[y][x] == 0:
    ...
    while True:
        ...
        while not game_over(board):
            ...
            if current_player == HUMAN_PLAYER:
                ...
                if is_valid_move(board, x, y, HUMAN_PLAYER):
                    make_move(board, x, y, HUMAN_PLAYER)
                ...
                for y in range(BOARD_SIZE):
                    for x in range(BOARD_SIZE):
                        if is_valid_move(board, x, y, COMPUTER_PLAYER):
                            new_board = deepcopy(board)
                            make_move(new_board, x, y, COMPUTER_PLAYER)
                            score = minimax(new_board, SEARCH_DEPTH, False, COMPUTER_PLAYER)
                            if score > best_score:
                                best_score = score
                                best_move = (x, y)
                if best_move:
            ...
            if is_tie(board):
                message = "Neizšķirts. Vai vēlaties mēģināt vēlreiz?"
                break
